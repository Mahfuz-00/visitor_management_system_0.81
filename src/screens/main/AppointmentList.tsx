      ...
      {appointmentList.map((item, index) => (
        ...
        >
          <View style={styles.leftColumn}>
            ...
            <Text style={styles.time}>{moment(item.request.time, 'HH:mm').format('HH:mm A')}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  day: { fontWeight: 'bold', fontSize: 24, color: 'red' },
  month: { fontWeight: 'bold', fontSize: 24, color: 'red' },
  time: { fontSize: 16 },
  rightColumn: { width: '70%', flexDirection: 'row', alignItems: 'center' },
  avatar: { width: 50, height: 50 },
  info: { marginLeft: 10, flex: 1 },
  name: { fontSize: 20, fontWeight: 'bold', color: 'green' },
  type: { fontSize: 14, marginTop: 10 },
  pending: { fontSize: 14, marginTop: 5, color: 'red' },
