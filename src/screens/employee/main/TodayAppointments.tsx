...
export default function TodayAppointments({ navigation }: any) {
  ...
  const getTodayAppointments = async () => {
    ...
    try {
      const res = await getData(`employee/appointment/today?page=${currentPage}`, auth.token!);
      if (res.data) {
  ...
  useEffect(() => {
    if (isFocused) getTodayAppointments();
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={{ marginTop: 10, color: '#999', fontSize: 16 }}>
          {/* ✅ Use a ternary operator for the language */}
          {language === 'EN' ? 'No Appointments Today' : 'আজ কোনো অ্যাপয়েন্টমেন্ট নেই'}
        </Text>
      </View>
    );
  }
        ...
        >
          <View style={styles.topRow}>
            ...
            <View style={styles.nameContainer}>
              <Text style={styles.name}>{item.from?.name || 'Unknown'}</Text>
...
// Reuse the styles from the Stylesheet in Section 1 (PendingRequest)
